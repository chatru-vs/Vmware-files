...
NI Virtual Infrastructure: Snapshot older by 1 day Status 
"@

foreach ($cluster in $NI_Clusters)
{
if($cluster.name -ne "Xen_Desktop_Cluster" -and $cluster.name -ne "OY_XenDesktop_Cluster" -and $cluster.name -ne "PG_XenDesktop_Cluster")
{
...
$mailto = "IOvm@news.co.uk,iounixteam@news.co.uk,iodatacentreteam@news.co.uk,iowindows@news.co.uk"
#$mailto = "sumit.chopra@news.co.uk"
#$mailto = "vijayarengan.ramachandran@news.co.uk"
$mailsubject = "NI Virtual Infrastructure: Snapshot older by 1 day Status - $date"
